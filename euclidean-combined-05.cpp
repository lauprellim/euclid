...
class hyperEuclid:public Euclid {
    ...
    vector<int> rotateHyperEuclid(int j) {
        for (int i = 0; i<hyperEuclidSet.size(); i++) {
            int num = ( ( hyperEuclidSet[i] + R[j] ) % NK[0] );
...
int main() {
    ...
    hyperEuclid basicHESet;

    basicHESet.setNK(NK, depth+1);
    basicHESet.setR(R, depth);
    basicHESet.setDepth(depth);
    ...
    */
    ...
    if(method=='r') basicHESet.makeHyperEuclidRotateFirst();
    else basicHESet.makeHyperEuclidSubtractFirst();
