...
class Euclid {
    ...
    // do I need a setter for a public variable??
...
class hyperEuclid : public Euclid {
    ...
    void setNK( int* new_NK, int depth ) {
        // allocate size of NK[] array first and initialize all valeus to 0
        NK = new int[depth];
        // NK[depth] = {};
    ...
    void setR( int* new_R, int depth ) {
        R = new int[depth+1];           // not sure this needs to be depth+1...
        // R[depth+1] = {};
    ...
    // do I need these, as they are public variables??
    ...
    void makeHyperEuclid() {
        ...
        // checking NK and R values...
        // cout<<endl; printNK(); printR(); cout<<endl;
        ...
        for(int j=0; j<depth; j++) {
            ...
            // myInstance[j].setR(R[j+1]);      // this was for ROTATE FIRST.
            ...
            // cout<<"R[j+1] is: "<<R[j+1]<<endl;     // <--- there is definitely sometihng wrong here for the last set
...
int main() {
    cout << boldOn << "FIND A HYPER-EUCLIDEAN SET (SUBTRACT FIRST METHOD)" << boldOff << endl;
    ...
    NK[depth] = {};
    int *R; R = new int[depth+1];     // there is a problem here -- depth+1 doesn't seem to solve the problem
    R[depth+1] ={};
